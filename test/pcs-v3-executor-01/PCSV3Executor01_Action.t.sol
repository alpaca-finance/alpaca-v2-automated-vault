...
contract PCSV3Executor01RepurchaseForkTest is PCSV3Executor01ActionTest, BscFixture {
  ...
  function testCorrectness_Repurchase_VaultManagerIsCaller() public {
    vm.mockCall(
      mockBank,
      abi.encodeWithSignature("borrowOnBehalfOf(address,address,uint256)", mockVaultToken, address(usdt), 1 ether),
      abi.encode()
    );
    ...
    vm.mockCall(
      ...
      abi.encodeWithSignature(
        "repayOnBehalfOf(address,address,uint256)", mockVaultToken, address(wbnb), 3068419005692078
      ),
      abi.encode()
    );
  ...
  function testRevert_Repurchase_CallerIsNotVaultManager() public {
    vm.prank(address(1234));
    vm.expectRevert(Executor.Executor_NotVaultManager.selector);
    executor.repurchase(address(mockToken0), 1e18);
  }
}
